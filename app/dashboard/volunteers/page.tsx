...
const ROLES = ["Todos los roles", "Instructor", "Facilitador", "Asistente", "Mentor", "Moderador", "Coordinador"];
...
export default function VolunteersPage() {
  ...
  const approve = (id: string) =>
    setList((prev) => prev.map((v) => (v.id === id ? { ...v, status: "aprobado" } : v)));
  const reject = (id: string) =>
    setList((prev) => prev.map((v) => (v.id === id ? { ...v, status: "rechazado" } : v)));
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((v) => (
                <tr key={v.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-4">
                    <span className="text-sm font-bold text-slate-900">{v.hours > 0 ? `${v.hours}h` : "—"}</span>
                  </td>
                  <td className="px-4 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {v.status === "aprobado" && (
                        <button className="px-3 py-1.5 rounded-lg bg-slate-100 text-slate-600 text-xs font-bold hover:bg-[#f49d25]/10 hover:text-[#f49d25] transition-colors flex items-center gap-1">
                      ...
                      {v.status === "rechazado" && (
                        <button
                          onClick={() => approve(v.id)}
                          className="px-3 py-1.5 rounded-lg bg-slate-100 text-slate-600 text-xs font-bold hover:bg-emerald-50 hover:text-emerald-700 transition-colors"
                        >
                          Reactivar
                        </button>
                      )}
                      <button className="p-1.5 rounded-lg bg-slate-100 text-slate-500 hover:bg-[#f49d25]/10 hover:text-[#f49d25] transition-colors">
